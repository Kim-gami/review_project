import os, dotenv, html, folium, sqlite3, ast, re
import pandas as pd
...
# GOOD_SHOPS = [
#     '토속정','이남장 분당점','더진국 분당정자역점','맛찬들왕소금구이 분당정자점','만족오향족발 정자점','샬라타이',
...
def render_compact_store_card(row: dict):
    ...
    """
    components.html(html_block, height=400, scrolling=False)
    ...
    (function(){
      ...
      navigator.geolocation.getCurrentPosition(
        function(pos){
          ...
          // ✅ 전체 reload 대신 streamlitRerun 이벤트 발행
          window.parent.dispatchEvent(new Event("streamlitRerun"));
        },
        function(_err){},
...
def _apply_tag(tag_text: str):
    ...
    st.session_state["kw_input_hidden"] = tag_text
    st.session_state["search_kw"] = tag_text
    st.session_state["do_search"] = True
    st.session_state["search_in_progress"] = True

    # 이미 URL kw를 적용했다고 표시해서 상단의 '한번만 적용' 로직과 충돌하지 않게 함
    st.session_state["_url_kw_applied"] = True
    ...
    new_q = {}
    for k, v in qp.items():
        # st.query_params 값은 리스트로 올 수 있으니 안전하게 첫값 사용
        new_q[k] = v[0] if isinstance(v, (list, tuple)) and v else v

    new_q["kw"] = tag_text
    new_q.pop("action", None)
    st.query_params.clear()
    st.query_params.update(new_q)

    # experimental_set_query_params 대신 기존 API 사용 (mix 금지)
    st.query_params.clear()
    st.query_params.update(new_q)
...
def _apply_kw():
    ...
    if "kw_input_hidden" not in st.session_state:
        st.session_state["kw_input_hidden"] = input_value
#메인
#위치 확보
...
geo = kakaoapi.get_gu_dong(BASE_LAT, BASE_LON)
...
with left:
    st.markdown(
        """
        <a href="/" target="_self" class="brand" style="
            text-decoration:none;
            color:#FF6B35;   /* 배너와 유사한 주황 계열 */
            font-size: 36px; /* 폰트 크기 키움 */
            font-weight: bold;">
        PickPlace
        </a>
        """,
    ...
    st.markdown(
        ...
        <div style="display:flex; justify-content:flex-end; gap:10px; align-items:center;">
          <button id="btn-open-loc" class="loc-btn" style="
            cursor:pointer; border:1px solid {PRIMARY}; padding:6px 12px; border-radius:12px;
            color:{PRIMARY}; background:#FDF6EC; font-weight:700; font-size:15px;"
            onclick="(function(){{
...
st.markdown(f'''
<div class="hero">
  <h3 style="margin:0 0 10px 0; font-size:32px;">오늘은 어떤 맛집을 찾고 계신가요?</h3>
  <p style="font-size:24px; opacity:0.9;">현위치 : {gu} {dong}</p>
</div>
''', unsafe_allow_html=True)
...
<div style="display:flex; gap:8px; margin-bottom:16px;">
    <input type="text"
           ...
           value="{html.escape(st.session_state.get('kw_input', ''))}"
...
if st.session_state.get("do_search", False):
    ...
    for i in range(rows_n):
        ...
        for j in range(n_cols):
            ...
            if complain_list:
                complain_html = "<ul style='margin:6px 0 0 18px; padding:0; font-size:15px;'>" + \
                                "".join(f"<li style='color:white !important;'>{html.escape(str(c))}</li>" for c in
                                        complain_list[:3]) + \
                                "</ul>"
            else:
                complain_html = "<div class='meta' style='margin-top:4px; color:white !important;'>아쉬운 점 없음</div>"
            ...
            with cols[j]:
                ...
                with c1:  # This is the left/wider column: place the card body here
                    st.markdown(f"""
                        <div class="card-body">
                          <h4>{name}</h4>
                          <div class="rating-info meta" style="margin-bottom:6px;">AI 추천 평점 {rating_str}</div>
                          <div class="stars" style="margin:4px 0;">{star_html}</div>

                          <div style="font-size:16px; color:#333; background:#f9f9f9; padding:8px;
                          ...
                          <div style="margin-top:8px;">
                            <div class="meta" style="margin-bottom:4px;">AI 분석 아쉬운 점</div>
                            {complain_html}
                          </div>
                        </div>
                        """, unsafe_allow_html=True)
                ...
                if st.session_state.get(f"show_reviews_{name}", False):
                    ...
                    if rows:
                        ...
                        review_items_html = ""
                        for (rv,) in rows:
                            # html.escape를 사용해 안전하게 변환합니다.
                            review_items_html += f"<li style='margin-bottom: 8px;'>{html.escape(rv)}</li>"

                        # 2. 스크롤 가능한 div 박스로 감싸서 한 번에 출력합니다.
                        scrollable_reviews_html = f"""
                        <div style="
                            max-height: 400px; /* 박스의 최대 높이를 400px로 제한 */
                        ...
                        """
                        st.markdown(scrollable_reviews_html, unsafe_allow_html=True)
                    else:
                        st.info("저장된 리뷰가 없습니다.")

gsdf = pd.read_csv("모범음식점_정제본.csv")
search_area = None  # 동/가 공통 토큰

if st.session_state.get("do_search", False) and not data.empty:
    addr_first = str(data.iloc[0]["store_address"] or "")

    #~동
    m = re.search(r'([가-힣·]+동)(?=\s|$|[0-9-])', addr_first)
    if m:
        search_area = m.group(1)

    #~가
    if not search_area:
        m = re.search(r'([가-힣·]+(?:\d+)?가)(?=\s|$|[0-9-])', addr_first)
        if m:
            search_area = m.group(1)

current_area = search_area or dong
GOOD_SHOPS = gsdf[gsdf["소재지주소"].str.contains(current_area, na=False)]["업소명"].tolist()
